...
void CMyDrawView::OnBeginPrinting(CDC* pDC, CPrintInfo* pInfo)
{
    ...
    // Accept doouble to int truncation (cuts off fractional units).
...
void CMyDrawView::DoHitTesting(CPoint point)
{
    ...
    while(pDoc->GetPos() != NULL /*&& !bSelectionMade */)
    {
        ...
        //  higher layer (higher shape overlaps current).
        if(!bSelectionMade && rect.PtInRect(point))
        {
            //m_pSelectedShape = pShape;
            // Tell shape it has been selected.
            pShape->m_bSelected = true;
            // Once a selection has been made, no other 
			//  is possible (but we still need to turn off 
			//  selection in rest of shapes).
            bSelectionMade = true;
        }
        else
        {
    ...
    {
        pDoc->UpdateAllViews(NULL);  // Moved out of while 
		                             //  for selection
    }
}
